"""Authentication routes: register, login, verify, forgot/reset password, Google OAuth."""
...
from fasthtml.common import (
    Html, Body, Div, H2, P, A, Button, Form, Input,
)
...
from utils.session import set_user_email, set_user_id, clear_user

log = logging.getLogger(__name__)

GOOGLE_CLIENT_ID = os.getenv("GOOGLE_CLIENT_ID", "")
GOOGLE_CLIENT_SECRET = os.getenv("GOOGLE_CLIENT_SECRET", "")
GOOGLE_REDIRECT_URI = settings().service_url.rstrip("/") + "/auth/google/callback"

SCHEMA = "main"  # SQLite default database
...
def register_auth_routes(rt):
        ...
        try:
            ...
            if existing:
                db.execute(text(f"""
                    ...
                    SET password_hash = :pw, name = :name, verify_token = :token, is_verified = 0
                    WHERE email = :email
    ...
    def auth_reset_page(token: str):
        ...
        try:
            row = db.execute(
                text(f"SELECT id FROM {SCHEMA}.chat_users WHERE reset_token = :token AND reset_token_expires > CURRENT_TIMESTAMP"),
                {"token": token},
            ).fetchone()
        finally:
            db.close()
        ...
        if not row:
            return Html(_head("Reset Password"), Body(
                Div(H2("Invalid or expired link"), P("Please request a new reset link."),
                    A("Back to app", href="/app", cls="text-black font-semibold"),
                    cls="max-w-md mx-auto mt-20 text-center"),
                cls="bg-white font-sans",
            ))
        ...
        return Html(_head("Reset Password"), Body(
            Div(
                ...
                Form(
                    ...
                    method="POST", action=f"/auth/reset/{token}/submit",
                ),
    ...
    async def auth_reset_submit(token: str, request, sess):
        ...
        try:
            row = db.execute(
                text(f"SELECT id, email FROM {SCHEMA}.chat_users WHERE reset_token = :token AND reset_token_expires > CURRENT_TIMESTAMP"),
                {"token": token},
            ).fetchone()
            if not row:
                return RedirectResponse("/app", status_code=303)
            ...
            db.execute(text(f"""
                UPDATE {SCHEMA}.chat_users
                SET password_hash = :pw, reset_token = NULL, reset_token_expires = NULL, is_verified = 1
                WHERE id = :id
    ...
    def auth_google_callback(request, sess):
        ...
        if error or not code or state != sess.get("oauth_state"):
            return RedirectResponse("/app", status_code=303)
        ...
        try:
            ...
            if row:
                ...
                db.execute(text(f"""
                    INSERT INTO {SCHEMA}.chat_users (email, name, is_verified) VALUES (:email, :name, 1)
